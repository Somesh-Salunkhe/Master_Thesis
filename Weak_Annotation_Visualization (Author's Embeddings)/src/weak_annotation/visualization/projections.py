...
def perform_tsne(
    ...
    """
    Performs t-SNE on features and centroids together.
    Returns: (features_2d, centroids_2d, subsample_indices)
    """
    X = features
    if X.shape[0] > max_points:
        indices = np.random.default_rng(random_seed).choice(X.shape[0], max_points, replace=False)
        X_sub = X[indices]
    else:
        X_sub = X
        indices = np.arange(X.shape[0])

    combined = np.vstack([X_sub, centroids])
    tsne = TSNE(
        n_components=2, 
...
def plot_combined_projection(
    ...
    """Creates a dual-plot figure showing cluster IDs and text labels in 2D space."""
    
    n_clusters = cen2d.shape[0]
    ...
    # Left Plot: Cluster IDs
    c_colors = [palette[int(cid) % len(palette)] for cid in cluster_ids]
    fig.add_trace(go.Scattergl(
        ...
        showlegend=False,
        hoverinfo="text",
        text=[f"Cluster {int(cid)}" for cid in cluster_ids],
    ), row=1, col=1)
    ...
    for i, lbl in enumerate(unique_labels):
        ...
        fig.add_trace(go.Scattergl(
            ...
            hoverinfo="text",
            text=[f"{lbl}<br>Cluster {int(cid)}" for cid in cluster_ids[mask]],
        ), row=1, col=2)
